  ...
  let segments_connect =
    if x_len == 0
      then y_lis
        ...
        then x_lcs
        else if pred2 x_last y_first
          then x_lcs + y_lis
          else 0

  let new_lss = segments_connect `max` x_lss `max` y_lss

  let new_lis =
    if x_len == 0 || x_lis == x_len && pred2 x_last y_first
      then x_len + y_lis  
      else x_lis
  
  let new_lcs = 
    if y_len == 0 || y_lcs == y_len && pred2 x_last y_first
      then y_len + x_lcs
      else y_lcs
